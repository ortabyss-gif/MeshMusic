  ...
  await Promise.all(
    Object.entries(zip.files).map(async ([name, entry]) => {
      ...
      if (['mp3','webp','png','jpg','jpeg'].includes(ext)) {
        ...
        files[baseName] = new Blob([blob], { type: mimeTypes[ext] || '' })  // ← asigna tipo
...
function Player({ playlist, currentIndex, onNext, onPrev, onLoadMore }) {
  ...
  // FIX: ref siempre actualizada de onNext para el evento 'ended'
  ...
  useEffect(() => {
    ...
    // console.log('Extrayendo colores de:', colorSrc ? `${colorSrc.type} · ${(colorSrc.size/1024).toFixed(1)}KB` : 'ninguno')
    if (colorSrc) {
  ...
  // Inyecta CSS en shadow DOM: oculta header + quita resaltado de foco en letras
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    TRACKS.forEach(name => {
      ...
      analyser.fftSize = 256
        analyser.smoothingTimeConstant = name==='vocals'?0.85:name==='bass'?0.45:0.5
      if (name==='vocals') {
    ...
    // FIX: usa la ref para que siempre llame al onNext más reciente
...
// DESPUÉS
      ...
      let other=0; for(let i=20;i<80;i++) other+=dataArrays.other[i]; other/=60
      ...
      if(Math.abs(iB-prevBass.current)>0.015||Math.abs(iV-prevVocals.current)>0.015){
        prevBass.current=iB; prevVocals.current=iV
        const base=baseColorsRef.current||FALLBACK_COLORS
        setColors(base.map((hex,i)=>{
          const rgb=hexToRgb(hex)
          if(i===0){const l=rgbToHex(lightenRgb(rgb,20));return lerpColor(hex,l,iV)}
          if(i===2||i===4){const l=rgbToHex(lightenRgb(rgb,20));return lerpColor(hex,l,iB)}
          return hex
        }))
      }
    }
      ...
      <div style={{position:'absolute',inset:0,zIndex:1,display:'flex',flexDirection:isMobile?'column':'row',alignItems:'stretch',padding:isMobile?'24px 20px 0':'0 60px',gap:isMobile?'16px':'60px'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',flexShrink:0,gap:'12px'}}>
          ...
          <div style={{
            ...
            gap:isMobile?'16px':'20px',backdropFilter:'blur(20px)',
            background:'rgba(0,0,0,0.2)',padding:isMobile?'16px':'28px',
            borderRadius:'28px',width:isMobile?'100%':'auto',boxSizing:'border-box',
          }}>

            {coverUrl && (
              <img src={coverUrl} alt="cover" style={{
                width:isMobile?'80px':'300px',height:isMobile?'80px':'300px',
                borderRadius:isMobile?'12px':'18px',objectFit:'cover',flexShrink:0,
              }}/>
            )}

            <div style={{display:'flex',flexDirection:'column',gap:'12px',flex:isMobile?1:'unset',width:isMobile?'0':'300px',minWidth:0}}>

              {/* Fila: título/artista/album + controles */}
...
export default function App() {
  ...
  // FIX: ref siempre actualizada de playlist para que handleNext no quede stale
